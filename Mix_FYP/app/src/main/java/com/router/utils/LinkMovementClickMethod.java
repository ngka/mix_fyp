...
/************************************************************
 * Description: .... spannable 中用於區別按一下和長按的（按兩下暫時不管）
 ***********************************************************/
public class LinkMovementClickMethod extends LinkMovementMethod {

    private long lastClickTime;

    private static final long CLICK_LONG_DELAY = 500l; //判斷是否長按的閾值
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_DOWN) {
            ...
            if (link.length != 0) {
                if (action == MotionEvent.ACTION_UP) {
                    // 判斷不是長按
                    if (System.currentTimeMillis() - lastClickTime < CLICK_LONG_DELAY) {
